...
export default function ControlView() {
  ...
  const [userText1, setUserText1] = useState<string>(
    "Click here and type some text"
  );
  const [userText2, setUserText2] = useState<string>(
    "Also click here and type some text"
  );
  const [userText3, setUserText3] = useState<string>(
    "If you click here you can type a lot of text"
  );
  ...
  return (
    ...
    >
      ...
      <button type="button" value="Submit All" onClick={submitAll}>
        Submit All
      </button>
      {editAll ? (
        <button type="button" onClick={editAllOff}>
          Edit All - Off
        </button>
      ) : (
        <button type="button" onClick={editAllOn}>
          Edit All - On
        </button>
      )}
    </div>
